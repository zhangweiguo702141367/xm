...
public class EmailServiceFo {
     ...
     * @throws Exception
     */
    public String getEmailPwdSign(String login_name,String email) throws Exception{
        if("".equals(login_name)){
            throw new Exception("登录名不能为空！");
        }
        if("".equals(email)){
        ...
        System.out.println("encode====sign==="+sign);
        sb = sb.append("?token=").append(URLEncoder.encode(token,"UTF-8")).append("&sign=").append(URLEncoder.encode(sign,"UTF-8"));
        return sb.toString();
    }

    /**
     * 根据token和sign解密邮箱认证token获取用户登录名和邮箱
     * @param token
     * @param sign
     * @return
     * @throws Exception
     */
    public Map<String,Object> designEmailPwd(String token,String sign) throws Exception{
        if("".equals(token)){
            throw new ProcException("token不能为空");
        }
        if("".equals(sign)){
        ...
        String resultString = new String(decodedData);
        Map<String,Object> signMap = JSON.parse(resultString,Map.class);
        String[] params = new String[]{"login_name","expire_time","create_time","email"};
        if(!RequestUtil.hasAllKey(signMap,params)){
            throw new ProcException("解密失败");
        }
        Long current_time = DateUtil.getCurrentTime();
        Long create_time = Long.valueOf(signMap.get("create_time").toString());
        Long expire_time = Long.valueOf(signMap.get("expire_time").toString());
